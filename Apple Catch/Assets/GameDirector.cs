...
public class GameDirector : MonoBehaviour {
	...
	void Update () {
        ...
        {
            this.generator.GetComponent<ItemGenerator>().SetParameter(0.7f, -0.05f, 7);
        }
        else if (20 <= this.time && this.time < 30)
        {
            this.generator.GetComponent<ItemGenerator>().SetParameter(0.7f, -0.04f, 8);
        }

        this.timerText.GetComponent<Text>().text = this.time.ToString("F1");
        this.pointText.GetComponent<Text>().text = "HP: "+this.point.ToString();
    }
}
